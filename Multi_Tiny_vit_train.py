...
from data_loader.cutmix import CutMixCollator, CutMixCriterion
...
def train(data_dir, model_dir, args):
    ...
    for i, (train_idx, valid_idx) in enumerate(skf.split(dataset.image_paths, labels)):
        ...
        wandb.run.name = f'aug_cutmix_age*4_DeepHeadMultiTinyViT_fold{i}'
        ...
        # )
        if args.augmentation == "CutmixAugmentation":
                train_loader, val_loader = getDataloader_cutmix(
                    dataset, train_idx, valid_idx, batch_size, num_workers, args.cutmix
                )
        else:
                train_loader, val_loader = getDataloader(
                    dataset, train_idx, valid_idx, batch_size, num_workers
        ...
        fold_dir = increment_path(os.path.join(save_dir, f"fold{i}"))
        ...
        model.load_state_dict(torch.load("/data/ephemeral/home/level1-imageclassification-cv-04/v3/K-Fold/best.pth"))
        # for param in model.head_mask.parameters():
        #     param.requires_grad = False

        # for param in model.head_gender.parameters():
        #     param.requires_grad = False
        
        # for param in model.head_age.parameters():
        #     param.requires_grad = False
        ...
        if args.criterion_gender =='focal':
        ...
        cutmix_criterion_mask = CutMixCriterion(criterion_mask)
        cutmix_criterion_gender = CutMixCriterion(criterion_gender)
        cutmix_criterion_age = CutMixCriterion(criterion_age)
                ...
                if args.multi_head:
                    ...
                    if args.augmentation == "CutmixAugmentation":
                        ...
                        with torch.cuda.amp.autocast():
                            ...
                            loss_mask = cutmix_criterion_mask(pred_mask, (mask, mask2, lam))
                            loss_gender = cutmix_criterion_gender(pred_gender, (gender, gender2, lam))
                            loss_age = cutmix_criterion_age(pred_age, (age, age2, lam))
                            loss = loss_mask + loss_gender + 4*loss_age
                    else:
                        mask = mask.to(device)
                        gender = gender.to(device)
                        age = age.to(device)
