...
class Multi3HeadTinyViT(nn.Module):
    def __init__(self, num_classes_mask=3, num_classes_gender=2, num_classes_age=3):
        super().__init__()
        self.model = tiny_vit_21m_224(pretrained=True, num_classes=0)
        #parameters freeze
        ...
        self.head_mask = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, num_classes_mask)
            )
        self.head_gender = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, num_classes_gender)
            )
...
class Multi2HeadTinyViT(nn.Module):
    def __init__(self, num_classes_mask_gender=6, num_classes_age=3):
        super().__init__()
        self.model = tiny_vit_21m_224(pretrained=True, num_classes=0)
        #parameters freeze
        # for param in self.model.parameters():
        ...
        embed_size = 576
        self.norm_head = nn.LayerNorm(embed_size)
        self.head_mask_gender = nn.Sequential(
            nn.Linear(embed_size, 1024),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(),
            nn.Dropout(0.5),
            ...
            nn.LeakyReLU(),
            nn.Dropout(0.5),
            nn.Linear(512, num_classes_mask_gender)
            )
    ...
    def forward(self, x):
        ...
        # 각 head를 통과하여 예측 수행
        mask_gender = self.head_mask_gender(x)
        age = self.head_age(x)

        return mask_gender, age
    



"""
class MultiHeadTinyViT(nn.Module):
    def __init__(self, img_size=(224,224), in_chans=3, num_classes_mask=3, num_classes_gender=2, num_classes_age=3, # num_classes=0
        ...
        # self.head_mask = nn.Linear(embed_dims[-1], num_classes_mask)
        self.head_mask = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(256, num_classes_mask)
            )

        # self.head_gender = nn.Linear(embed_dims[-1], num_classes_gender)
        self.head_gender = nn.Sequential(
            nn.Linear(embed_dims[-1], 512),
            nn.ReLU(),
            nn.Dropout(0.5),
        ...
        # self.head_age = nn.Linear(embed_dims[-1], num_classes_age)
        self.head_age = nn.Sequential(
            nn.Linear(embed_dims[-1], 512),
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def set_layer_lr_decay(self, layer_lr_decay):
        ...
        for m in [self.norm_head, self.head_mask, self.head_gender, self.head_age]:
            m.apply(lambda x: _set_lr_scale(x, lr_scales[-1]))
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=18,
                 embed_dims=[96, 192, 384, 768], depths=[2, 2, 6, 2],
                 num_heads=[3, 6, 12, 24],
                 window_sizes=[7, 7, 14, 7],
                 mlp_ratio=4.,
...
"""
@register_model
def tiny_vit_5m_224(pretrained=True, **kwargs):
    model_kwargs = dict(
