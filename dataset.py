...
class CustomAugmentation:
    ...
    def __init__(self, resize, mean, std, **args):
        ...
        self.transform = A.Compose([
            A.CenterCrop (360, 360, always_apply=False, p=0.5),
            A.Resize(*resize),
...
class MaskBaseDataset_2Head(Dataset):
    """마스크 데이터셋의 기본 클래스"""
    ...
    def __getitem__(self, index):
        ...
        age_label = self.get_age_label(index)
        image_transform = self.transform(image)
        mask_gender_class, age_class = self.encode_multi_class(mask_label, gender_label, age_label)

        if self.multi_head:
            return image_transform, (mask_gender_class, age_class), mask_label, gender_label, age_label
            
        else:
            return image_transform, self.encode_multi_class(mask_label, gender_label, age_label)
    ...
    @staticmethod
    def encode_multi_class(mask_label, gender_label, age_label) -> Tuple[int, int]:
        """마스크와 성별 조합을 하나의 클래스로 인코딩하고, 나이는 별도로 인코딩"""
        mask_gender_class = mask_label * 2 + gender_label
        return mask_gender_class, age_label
    
    @staticmethod
    def decode_multi_class(
        mask_gender_class,
        age_class
        ) -> Tuple[Tuple[MaskLabels, GenderLabels], AgeLabels]:
        """인코딩된 마스크와 성별 조합, 나이 라벨을 각각의 라벨로 디코딩"""
        mask_label = mask_gender_class // 2
        gender_label = mask_gender_class % 2
        age_label = age_class
        return (mask_label, gender_label), age_label
