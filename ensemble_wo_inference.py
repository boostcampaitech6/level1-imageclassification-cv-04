...
def main(config):
    ...
    submissions = glob.glob(f'{config.submissions_dir}/*.csv')

    if config.ensemble == 'hard':
        models = []
        hvs = []
        for s in submissions:
            run_name = os.path.splitext(os.path.basename(s)[11:])[0]
        ...
        df_hard_voting = pd.DataFrame.from_dict({k:v for k, v in zip(models, hvs)})
        ...
        if not os.path.exists(os.path.join(config.submissions_dir, '/ensemble_hv')):
            os.mkdir(os.path.join(config.submissions_dir, '/ensemble_hv'))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--ensemble", type=str, default="hard", help="ensemble type (default: hard)"
    )
    parser.add_argument(
        "--test_dir",
        type=str,
